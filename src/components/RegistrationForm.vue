<template>
  <a-form-model @submit="submit" @submit.native.prevent ref="ruleForm" :model='form' :rules="rules" class='auth-form' :class='$mq'>
    ...
    <a-form-model-item>
      <a-button type="primary" html-type="submit" :disabled="form.email === '' || form.password === '' || form.name === ''">
        Регистрация
      </a-button>
    </a-form-model-item>
  </a-form-model>
      ...
      return {
        ...
        rules: {
          email: [
            ...
            { type: 'email', message: 'Не корректный Email адресс!', trigger: 'blur'}
          ],
          password: [
            { required: true, message: 'Введите пароль!', trigger: 'blur' },
            { min: 6, max: 12, message: 'Длина должна быть от 6 до 12', trigger: 'blur' },
          ]
        }
      }
    },
